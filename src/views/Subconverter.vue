      ...
      <el-col>
        <el-card style="margin-top:20px;max-width:800px;margin:auto;opacity:0.8;blackground-color:#0F4677;border-radius: 20px;">
          <div slot="header" style="blackground-color:#0F4677;text-align:center;font-size :25px !important;font-weight: bold !important;">
            <svg-icon icon-class="lock" style="margin-left: 20px" title="Hoàn thành tái thiết ma thuật:v1.4"/>
            AikoCuteHotMe
            <svg-icon icon-class="telegram" style="margin-left: 10px" title="Thêm vào Telegram" @click="gotoTgChannel" />
          </div>
          <el-container>
            <el-form :model="form" label-width="80px" label-position="left" style="width: 100%;">
              <el-form-item label="Tùy chọn nâng cao:">
                ...
                <el-radio v-model="advanced" label="1">Chế độ cơ bản</el-radio>
                <el-radio v-model="advanced" label="2">Chế độ nâng cao</el-radio>
              </el-form-item>
              <el-form-item label="Liên kết đăng ký:">
                <el-input
                  v-model="form.sourceSubUrl"
                  type="textarea"
                  rows="3"
                  placeholder="Hỗ trợ đăng ký hoặc liên kết SS/SSR/VMESS/Trojan, nhiều liên kết mỗi dòng hoặc sử dụng | riêng biệt"
                  @blur="saveSubUrl"
                />
              </el-form-item>
              <el-form-item label="Thế hệ:">
                ...
                <el-form-item label="Quy tắc từ xa:">
                  <el-select
              ...
              <el-form-item label="Địa chỉ trở lại:">
              ...
              <div v-if="advanced === '2'">
                ...
                <el-form-item label="Bao gồm các nút:">
                  <el-input v-model="form.includeRemarks" placeholder="Từ khóa chứa tên nút, hỗ trợ thường xuyên" />
                </el-form-item>
                <el-form-item label="Loại bỏ các nút:">
                  <el-input v-model="form.excludeRemarks" placeholder="Tên nút không chứa từ khóa, hỗ trợ thông thường" />
                </el-form-item>
                <el-form-item label="Tên đầu ra:">
                  <el-input v-model="form.filename" placeholder="Quay lại đăng ký tên tệp" />
                </el-form-item>
                <el-form-item label-width="0px">
                  <el-row type="flex">
                    <el-col>
                      <el-checkbox v-model="form.nodeList" label="Đầu ra Node List" border></el-checkbox>
                      <el-checkbox v-model="form.emoji" label="Emoji" border></el-checkbox>
                      <el-checkbox v-model="form.sort" label="Sắp xếp nút" border></el-checkbox>
                      <el-checkbox v-model="form.udp" @change="needUdp = true" label="Mở UDP" border></el-checkbox>
              ...
              <el-form-item label="Đăng kí tuỳ chỉnh:">
              ...
              <el-form-item label="Đăng ký một chuỗi ngắn:">
...
export default {
  data() {
    var data = {
      ...
      options: {
        clientTypes: {
          ...
          V2Ray: "v2ray",
          Trojan: "trojan",
          "Đăng kí hỗn hợp（mixed）": "mixed",
          "Tự động đánh giá khách hàng": "auto",
        },
        customBackend: {
          "Kết thúc (IPv4/IPv6 cân bằng tải trọng tải sáu lõi)": "https://api.tsutsu.one/sub?",
        },
        backendOptions: [
        ...
        remoteConfig: [
          {
            ...
            options: [
              ...
              {
                label: "Lựa chọn tự động theo nhóm đầy đủ",
                value:
                  "https://cdn.jsdelivr.net/gh/lhl77/sub-ini@main/tsutsu-full-urltest.ini"
              },
              {
                label: "Super JB sắp xếp hợp lý bao gồm chuyển hướng trong nước",
                value:
          ...
          },
	{
          ...
          {
            label: "Người dùng gửi, gửi, vui lòng tìm @AikoCute",
            options: [
              {
                label: "HOPE140 Cấu hình tự sử dụng (đồng bộ hóa với GitHub)",
                value:
                  "https://cdn.staticaly.com/gh/hope140/Clash/beta/hope140.yaml"
              },
              {
                label: "Hope140 đi đến cấu hình chặn quảng cáo",
                value:
              ...
              {
                label: "Yuki Ẩn dân lượng IP trong nước theo chủ nghĩa tối thiểu",
                value:
  ...
  created() {
    ...
    document.title = "Đăng ký chuyển đổi";
  ...
  methods: {
    ...
    clashInstall() {
      if (this.customSubUrl === "") {
        this.$message.error("Vui lòng điền vào mục yêu cầu trước để tạo liên kết đăng ký");
        return false;
      }
    ...
    makeUrl() {
      if (this.form.sourceSubUrl === "" || this.form.clientType === "") {
        this.$message.error("Đăng ký liên kết và khách hàng được yêu cầu");
        return false;
      }
      // 远程接口
      let backend =
      ...
      if (sourceSub.indexOf("losadhwse") !== -1 && (backend.indexOf("py6.pw") !== -1 || backend.indexOf("subconverter-web.now.sh") !== -1 || backend.indexOf("subconverter.herokuapp.com") !== -1 || backend.indexOf("api.wcc.best") !== -1)) {
        this.$alert('Đăng ký sân bay này đã bảo vệ mặt sau này, vui lòng chuyển đổi phần cuối.', 'Lời nhắc lỗi chuyển đổi', {
          confirmButtonText: 'Chuyển đổi lỗi',
          callback: action => {
            this.$message({
              type: 'error',
      ...
      this.$message.success("Đăng ký tùy chỉnh đã được sao chép vào bảng tạm");
    },
    makeShortUrl() {
      if (this.customSubUrl === "") {
        this.$message.warning("Vui lòng tạo liên kết đăng ký, sau đó nhận liên kết ngắn tương ứng");
        return false;
      }
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.Code === 1 && res.data.ShortUrl !== "") {
            ...
            this.$message.success("Liên kết ngắn đã được sao chép vào bảng tạm");
          } else {
            this.$message.error("Mua lại liên kết ngắn không thành công:" + res.data.Message);
          }
        })
        .catch(() => {
          this.$message.error("Lỗi truy cập liên kết ngắn");
        })
        .finally(() => {
          this.loading = false;
        });
    ...
    confirmUploadConfig() {
      if (this.uploadConfig === "") {
        this.$message.warning("Cấu hình từ xa không thể trống");
        return false;
      }
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.code === 0 && res.data.data !== "") {
            this.$message.success("Cấu hình từ xa được tải lên thành công và liên kết cấu hình đã được sao chép vào bảng tạm");
            ...
            this.$message.error("Tải lên cấu hình từ xa không thành công..");
          }
        })
        .catch(() => {
          this.$message.error("Tải lên cấu hình từ xa không thành công..");
